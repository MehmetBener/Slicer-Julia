...
# Bring in real geometry code from geometry_2d.jl
...
function set_config!(s::Slicer, key::String, valstr::String)
    ...
    elseif typ == Vector{String}
        if rng !== nothing
            ...
            # If no range constraints, just store it
            s.conf[key] = valstr
            badval = false
        end
    end
...
function slice_to_file!(s::Slicer, filename::String; showgui::Bool=false)
    ...
    # Example of computing some internal geometry widths
    ...
    for model in s.models
        # Suppose these methods exist:
        center_z = (model.points.maxz + model.points.minz)/2  # or however you want
        model.center!((s.center_point[1], s.center_point[2], center_z))
        model.assign_layers!(s.layer_h)
    end
    # Count how many layers total
