...
# Example of pulling in your needed modules
include("point3d.jl")
...
function _read_ascii_line(self::StlData, f::IO; watchwords::Union{Nothing,String}=nothing)
    ...
    if watchwords !== nothing
        ...
        # Return numeric part after skipping watchwords
        return [parse(Float64, w) for w in words[(length(expected)+1):end]]
    else
        # no watchwords; just parse floats
        return [parse(Float64, w) for w in words if occursin('.', w) || occursin('-', w) || occursin('e', w)]
    end
end
...
function _read_binary_facet(self::StlData, f::IO; quanta::Float64=1e-3)
    data = read(f, Float32, 12)  # 3 for normal + 3*(3 for vertex)
...
function _write_ascii_file(self::StlData, filename::String)
    open(filename, "w") do f
        write(f, "solid Model\n")
        for facet in sorted(self.facets)  # define `sorted` or collect(values(...)) as needed
            # ...
            # Each `facet::Facet3D` stores vertices in facet[1], facet[2], facet[3], normal in facet[4]
            v0 = facet[1]
            v1 = facet[2]
            v2 = facet[3]
            nrm = facet[4]
...
function _write_binary_file(self::StlData, filename::String)
    open(filename, "w") do f
        ...
        # ...
        # left as an exercise (similar to your code)
    end
end
