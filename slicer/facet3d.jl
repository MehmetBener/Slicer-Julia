...
mutable struct Facet3D
    ...
    norm::Vector  # using the Vector type from .vector
    count::Int
end
...
function is_clockwise(f::Facet3D)
    v1 = Vector(f.vertices[2] - f.vertices[1])
    v2 = Vector(f.vertices[3] - f.vertices[1])
